...
function renderWaypointPhotoList() {
    ...
    const cardsHtml = filteredEntries.map(entry => {
        ...
        const creator = String(entry?.creatorName || '').trim();
        const canEdit = isOwnedByCurrentCloudUser(entry?.creatorEmail);
...
function renderDocumentList() {
    ...
    if (!container) return;
    const sessionEmail = getCurrentCloudUserEmail();
    ...
    const categoryHtml = Array.from(grouped.entries()).map(([categoryLabel, subMap]) => {
        const subHtml = Array.from(subMap.entries()).map(([subcategoryLabel, entries]) => {
            const rows = entries.map(entry => {
                ...
                const canEdit = !sessionEmail || isOwnedByCurrentCloudUser(entry?.creatorEmail);
                const canDelete = !sessionEmail || isOwnedByCurrentCloudUser(entry?.creatorEmail);
                return `<div class="document-list-row">
                    ...
                    <div class="document-list-row__actions">
                        ...
                        ${canEdit ? `<button type="button" class="document-edit-btn" data-document-id="${escapeHtml(entry.id)}">${t('Modifier', 'Modificar')}</button>` : ''}
                        ${canDelete ? `<button type="button" class="document-delete-btn" data-document-id="${escapeHtml(entry.id)}">${t('Détruire', 'Destruir')}</button>` : ''}
                    </div>
                </div>`;
            }).join('');
...
function renderMaintenanceExpenses() {
    ...
    maintenanceExpenses.forEach((expense, index) => {
        ...
        if (isOwnedByCurrentCloudUser(expense?.creatorEmail)) {
...
function renderMaintenanceExpenseDetailPanel() {
    ...
    const canDeleteExpense = isOwnedByCurrentCloudUser(selectedExpense?.creatorEmail);
    const canEditExpense = isOwnedByCurrentCloudUser(selectedExpense?.creatorEmail);
...
function renderMaintenanceSupplierDetailPanel() {
    ...
    const canDeleteSupplier = isOwnedByCurrentCloudUser(selectedSupplier?.creatorEmail);
    ...
    updateBtn.disabled = !selectedSupplier || !canDeleteSupplier;
    updateBtn.addEventListener('click', async () => {
        if (!selectedSupplier || !canDeleteSupplier) return;
        const name = String(nameInput.value || '').trim();
        if (!name) return;
    ...
    if (canDeleteSupplier) actionRow.appendChild(updateBtn);
...
function renderMaintenanceBoard() {
    ...
    if (!board) {
        ...
        const schemaDeleteBtn = document.getElementById('maintenanceDeleteSchemaBtn');
        if (schemaDeleteBtn) schemaDeleteBtn.style.display = 'none';
    ...
    const schemaDeleteBtn = document.getElementById('maintenanceDeleteSchemaBtn');
    if (schemaDeleteBtn) {
        schemaDeleteBtn.style.display = isOwnedByCurrentCloudUser(board?.creatorEmail) ? '' : 'none';
    }
    ...
    maintenanceBoards.forEach((schemaBoard, schemaIndex) => {
        ...
        sortedAnnotations.forEach((annotation, index) => {
            ...
            const canDeleteAnnotation = isOwnedByCurrentCloudUser(schemaBoard?.creatorEmail);
            let deleteBtn = null;
            ...
            if (deleteBtn) controlsWrap.appendChild(deleteBtn);
...
function updateCloudAuthUi() {
    ...
    // Keep sign-in/up always clickable to allow recovering from stale or broken sessions.
    if (emailSignInBtn) emailSignInBtn.disabled = false;
    if (emailSignUpBtn) emailSignUpBtn.disabled = false;
...
function isOwnedByCurrentCloudUser(creatorEmail) {
    const sessionEmail = normalizeEmailForCompare(getCurrentCloudUserEmail());
    if (!sessionEmail) return true;
    const normalizedCreator = normalizeEmailForCompare(creatorEmail || '');
    if (!normalizedCreator) return true;
    return normalizedCreator === sessionEmail;
}
...
function saveRouteFromRouting() {
    ...
    if (hasLoaded) {
        const loadedRoute = saved[currentLoadedRouteIndex] || null;
        if (!isOwnedByCurrentCloudUser(loadedRoute?.creatorEmail)) {
            alert(t('Tu peux consulter cette route mais pas la modifier.', 'Puedes consultar esta ruta pero no modificarla.', 'You can view this route but cannot modify it.'));
            return;
        }
    }
