...
function updateRoutingTabAvailability() {
    ...
    const shouldEnable = !isAuthGateLocked() && hasLoadedRouteSelection();
...
function scheduleCloudLogbookPush({ includeNav = true, includeEngine = true } = {}) {
    ...
    cloudLogbookPushTimer = setTimeout(async () => {
        ...
        try {
            ...
            cloudLogbookPushPendingNav = false;
            cloudLogbookPushPendingEngine = false;
...
function captureNavGpsSample(sampleSource = 'gps-watch') {
    ...
    if (navGpsSessionSamples.length > 3600) {
        navGpsSessionSamples = navGpsSessionSamples.slice(navGpsSessionSamples.length - 3600);
    }
...
function appendAutoNavSessionSummaryEntry() {
    const sessionEntries = (Array.isArray(navGpsSessionSamples) ? navGpsSessionSamples : [])
...
function startNavigationLogging() {
    ...
    navGpsSampleTimerId = window.setInterval(() => {
        ...
        if (captured) {
            setNavLogStatus(t(`Log GPS actif · point capturé (${NAV_GPS_SAMPLE_INTERVAL_MS / 1000}s) · ${navGpsSessionSamples.length} point(s)`, `Log GPS activo · punto capturado (${NAV_GPS_SAMPLE_INTERVAL_MS / 1000}s) · ${navGpsSessionSamples.length} punto(s)`));
        }
    ...
    navWatchId = navigator.geolocation.watchPosition(
        position => {
            ...
            setNavLogStatus(t(`Log GPS actif · fix reçu · capture chaque minute · ${navGpsSessionSamples.length} point(s)`, `Log GPS activo · fix recibido · captura cada minuto · ${navGpsSessionSamples.length} punto(s)`));
        },
        error => {
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    function activateTab(tabName) {
        ...
        if (tabName === 'routing' && !hasLoadedRouteSelection()) {
            tabName = 'routes';
        }
